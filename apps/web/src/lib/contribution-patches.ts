...
export function buildContributionChanges(patch: ContributionPatch): ContributionChangesPayload {
...
function humanizePath(path: string): string {
  ...
  const mc = /^pageContent\.mainCharacters\.[^.]+\.description$/.exec(path);
  if (mc) return 'Character description';
...
export function normalizeContributionPatches(changes: unknown): ContributionPatch[] {
  ...
  if (Array.isArray(changes)) {
    return changes
      ...
      .filter((p): p is ContributionPatch => p !== null && Boolean(p.newValue || p.oldValue));
  }
